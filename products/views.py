...
from core.models.base import DatabaseHelper
from . import crud
...
async def get_products(session: AsyncSession = Depends(DatabaseHelper.session_dependency())):
    return await crud.get_products(session=session)

@router.get('/{product_id}', response_model=Product)
async def get_product_by_id(product_id: int, session: AsyncSession = Depends(DatabaseHelper.session_dependency())):
